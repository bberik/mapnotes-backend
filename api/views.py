...
from .permissions import CreateWorkspacePermission, ListWorkspacePermission, RetrieveUpdateDeleteWorkspacePermission, CreateNodeEdgePermission, RetrieveUpdateDeleteNodeEdgePermission
...
class WorkspaceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_anonymous:
            if self.action == "list":
                return None
            elif self.action in ["destroy", "update", "retrieve", "partial_update"]:
...
def upload_image(request):
    # Use the MultiPartParser to parse the uploaded image
    ...
    s3_client.upload_fileobj(uploaded_image, settings.AWS_STORAGE_BUCKET_NAME, file_path)
